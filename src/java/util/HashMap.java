...
public class HashMap<K,V> extends AbstractMap<K,V> implements Map<K,V>, Cloneable, Serializable {
    ...
    static final class TreeNode<K,V> extends LinkedHashMap.Entry<K,V> {
        ...
        //普通的Node节点转化为红黑树的时候，除了红黑树 需要的 父节点、左子树、右子树 引用外，
        //还维护了一个 节点插入顺序的前向指针，与原来 Node next指针 结合在一起，就形成了 双向指针
        //这样的设计，是为了在拆分红黑树的时候，通过双向链表遍历树节点。
        TreeNode<K,V> prev;    // needed to unlink next upon deletion 需要在下次删除时取消链接
